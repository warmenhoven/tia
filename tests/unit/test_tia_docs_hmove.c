...
/* Apply HMP0 = value, strobe HMOVE, drain a full scanline so the motion
 * completes, assert p0_pos moved by -signed_motion. HMOVE is pipelined 6
 * colour clocks, then one stuff per object every 4 colour clocks through the
 * (extended) HBLANK — up to 15 stuffs, so a full-scanline drain is required.
...
static int motion_test(uint8_t hmp0, int expected_delta, const char *label)
{
    ...
    for (k = 0; k < TIA_SCANLINE_CLOCKS; k++) tia_tick(&t);
