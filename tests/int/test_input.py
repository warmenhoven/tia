...
def test_console_switches_toggle_on_shoulder_buttons(core_path):
    ...
    Default boot value is A (the friendlier rest-position default).
    R press flips to B, L press restores A.  Pressing once flips the
    latch; holding doesn't chatter it."""
    rom = _build_swchb_echo_rom()
    states = [
        libretro.JoypadState(),            # 0 boot
        libretro.JoypadState(),            # 1 neutral -> default (Diff A/A + color)
        libretro.JoypadState(r=True),      # 2 press R -> Left Diff B  (flip away from default)
        libretro.JoypadState(r=True),      # 3 still held: no change
        libretro.JoypadState(),            # 4 released
        libretro.JoypadState(l=True),      # 5 press L -> Left Diff A  (back to default)
    ]
    driver = libretro.IterableInputDriver(input_generator=iter(states))
    session = (libretro.SessionBuilder
               .defaults(core_path)
    ...
    assert r_pressed != neutral,    "R press should flip left-diff away from A default"
